...
# This source only updates THIS short-lived start child (environment.json
# `start` is `bash .cursor/start.sh`). Later shells pick up the same file
# via the bashrc/profile hook ensure_cloud_display.sh installs; later
# `python3 tools/verify_all.py` loads it through tools/cloud_env.py.
# Never put secrets in ~/.pokewilds-cloud.env.
