...
# --- Menu-state expected strings (sidecar expected_regions.strings) ---

# Curated expected texts for the menu sweep shots' sidecars; render_introspection
# joins each against the live menu labels (ink rect, mode "box") and the vision
# lane's deterministic reviewer diffs them baseline-vs-fresh. Snapshot: {"party":
# [mon...], "egg": {}}. The 29 extension (Egg summary) is the banked deterministic
# answerer for the menu rubric group's question set.
static func menu_expected_texts(state: String, snapshot: Dictionary) -> Array:
	var texts := []
	for mon in snapshot.get("party", []):
		texts.append(str((mon as Dictionary).get("name", "")))
	match state:
		"party_egg_summary": # 29: the Egg summary panel's first + countdown lines
			var egg: Dictionary = snapshot.get("egg", {})
			if not egg.is_empty():
				texts.append("Egg — %s" % str(egg.get("display_name", "?")))
				texts.append("Steps to hatch: %d" % int(egg.get("steps_to_hatch", 0)))
	return texts.filter(func(text): return not str(text).is_empty())
