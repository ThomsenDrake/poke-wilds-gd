...
# at the app-220 wall (a static run(sweep) that reaches into the sweep node for the shared
# plumbing). The origin satellite (visual_sweep_world_depth) captures (34) the Heart Tower
# footprint/interior here — the largest landmark, dedicated tileset, zero prior pixel coverage —
# mirroring 31/32's footprint-probe + tile-prop assert pattern (the tower has NO encounter token,
# so it asserts landmark_id + region + the rendered wall/decor props). This file also hosts the
# shared helpers the showcase guardian shot reuses: set_entities_active + the two-sample rest_state
# probe (the origin sweep's _rest_state was lifted here so the sweep stays at its wall) + the
# walkable_near camera framing. (Infinite-world slice 1 RETIRED the chained-world/guardian
# satellite; slice 3 deleted its now-orphaned pick_legendary / screen_tile_rect helpers.) NO rng.
...
# Runs the origin satellite's appended shot (the heart tower) on the sweep node. Loud-fails
# (never silent) when the anchor seam breaks.
...
# restore it. Shared with the showcase guardian shot (it opts in for its capture).
