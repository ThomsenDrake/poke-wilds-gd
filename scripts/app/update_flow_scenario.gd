...
func run(ctx: Dictionary) -> void:
	...
	await SmokeTap.tap(get_tree(), "action_a")
	await SmokeTap.tap(get_tree(), "action_a")
	await _wait_until(func(): return _runner.trace_log_has_since("update_apply_failed", 0))
	_check(not _applied, "hash mismatch still applied")
	_check(_runner.trace_log_has_since("update_apply_failed", 0), "hash mismatch did not trace update_apply_failed")
	_download_error = ""
	title.select_entry(0)
	await SmokeTap.tap(get_tree(), "action_a")
	await SmokeTap.tap(get_tree(), "action_a")
