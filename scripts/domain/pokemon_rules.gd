...
func resolve_gender(gender_ratio: String, seed_hint: String = "") -> String:
	...
	if not ratio.begins_with(GENDER_RATIO_PREFIX):
		return GENDERLESS
	var female_chance = clampf(ratio.substr(GENDER_RATIO_PREFIX.length()).replace("_", ".").to_float() / 100.0, 0.0, 1.0)
	if female_chance <= 0.0: return GENDER_MALE
	if female_chance >= 1.0: return GENDER_FEMALE
	var roll = float(absi(hash(seed_hint + ratio) % 1000)) / 1000.0
	return GENDER_FEMALE if roll < female_chance else GENDER_MALE

# A stored gender is only legal when the species ratio can produce it.
func _gender_compatible(gender: String, gender_ratio: String) -> bool:
	var ratio = gender_ratio.strip_edges().to_upper()
	if not ratio.begins_with(GENDER_RATIO_PREFIX):
		return gender == GENDERLESS
	var chance = ratio.substr(GENDER_RATIO_PREFIX.length()).replace("_", ".").to_float()
	if chance <= 0.0: return gender == GENDER_MALE
	if chance >= 100.0: return gender == GENDER_FEMALE
...
func build_stats(base_stats: Dictionary, level: int) -> Dictionary:
	...
	return {
		...
		"spe": max(1, int(floor((2.0 * int(base_stats.get("spe", 45)) * level) / 100.0)) + 5),
		"sat": max(1, int(floor((2.0 * int(base_stats.get("sat", 49)) * level) / 100.0)) + 5),
...
func build_move_set(move_ids: Array, move_lookup: Callable) -> Array:
	...
	for move_id_variant in move_ids:
		var move_entry = move_lookup.call(str(move_id_variant))
...
func normalize_loaded_mon(raw_mon: Dictionary) -> Dictionary:
	...
	mon["sleep_turns"] = 0
	for key in BattleStatus.VOLATILE_KEYS:
		mon.erase(key)

	mon["gender_ratio"] = str(mon.get("gender_ratio", ""))
	var gender = str(mon.get("gender", "")).strip_edges().to_lower()
	mon["gender"] = gender if _gender_compatible(gender, str(mon["gender_ratio"])) else resolve_gender(
		str(mon["gender_ratio"]), "%s:%d" % [str(mon.get("species_id", "")), int(mon["level"])])
...
func _collect_move_ids_for_level(species_entry: Dictionary, level: int) -> Array:
	...
	if learnset is Array:
		for entry_variant in learnset:
			...
			if int(entry.get("level", 1)) <= level and not move_id.is_empty() and not candidates.has(move_id):
