...
**Status**: **Build 2 of 3 LANDED 2026-07-30 (legendaries)** — Build 1 (landmarks) LANDED 2026-07-29 (Verify-phase godot confirmation pending): `scripts/domain/landmarks.gd` (the frozen chain-parametric generator seam: `tile_logic_for`/`landmarks_in_world`/`anchor_for`, all three landmarks in ANY world from day one, pure SplitMix anchors `SALT_LANDMARK_ANCHOR 0x23`, `ANCHOR_SEARCH_BUDGET := 400` + fixed-ring fallback; the Mansion puzzle as pure transitions), `scripts/runtime/landmark_runtime.gd` (entry detection, puzzle interaction via the FROZEN state seam `landmark_state_for(chain)`/`set_landmark_state(chain, state)` — resolved to the v4-additive top-level `landmark_state` key, NO `SAVE_VERSION` bump; footprint encounter scope → `generate_wild_encounter`), the NEW `scripts/runtime/session_payload.gd` extraction (session_state marshalling), `scripts/app/landmark_actions.gd` + `landmark_flow_scenario.gd` + `world_depth_checks.gd` + `visual_sweep_world_depth.gd`. Co-mods: `world_generator.gd`'s single mutation-boundary resolver hook (paid by extraction), `biome_encounters.gd`'s footprint-local `landmark_token` path, `structures.can_place_on`'s landmark-tile refusal, `field_action_router.gd`'s arms extracted, the Ruins-Underground DUSCLOPS via the Phase-6 stationary-entity mechanism, the `check_repo_contracts` rng ban extended to `scripts/domain/**` + `landmark_runtime.gd`. `landmark_flow` (SEED 2026072907, Mansion puzzle SOLVED on the fixed seed) JOINS the double-run lane, regrouped **[4, 3] interim** (Build 3's `world_chain` completes the spec's final [4, 4]); the `visual_sweep_world_depth` satellite baselines `31_ruins_interior` + `32_mansion` (`33_beacon` is Build 3's). Heart Tower INTERNALS DEFERRED (spec § Scope quarantine). **Build 2 (legendaries) LANDED 2026-07-30** — `scripts/domain/legendary_placement.gd` (the frozen seven `LEGENDARY_IDS`, flagged SNOW/LAVA affinity, pure `_mix` ring-≥60 anchors bounded to `LEGENDARY_ANCHOR_BUDGET := 300`, the `"<cx>,<cy>:<SPECIES>"` removal grammar + `legendaries_for_world` suppression); the frozen-seven stationary stamping (`overworld_mons_sim.stamp_legendaries` via the `overworld_mons_runtime` delegator — CLASS_STATIONARY, forced AGGRESSIVE, the guardian's +3 chase-catch); the `battle_kind` pending seam (`main.gd:92` → `play_battle_track("legendary")`, the first caller of `music_router`'s `BATTLE_TRACKS["legendary"]`; router unchanged, headless never plays audio); the `legendary_encounter` battle-start trace (single owner `game_runtime`); and the KO/catch removal onto the v4-additive `legendary_removals` key (white-out leaves the mon re-battleable, Phase-6 precedent). Co-mods under their homes: `biome_encounters`' `LEGENDARY_IDS` never-encounter exclusion (world_runtime), `session_state`/`session_payload`'s `legendary_removals` marshalling (session_runtime), two at-wall EXTRACTIONS (`entity_markers.gd`, `wild_encounter_draw.gd`). `legendary_spawn` (PIN 2026073001, `seed_for_smoke` BEFORE `new_game`) proves the ring gate, the never-encounter exclusion over every pool path, the chase-catch +3 + `battle_kind` seam, and the white-out-re-battleable / KO-gone-for-good rematch rules — and JOINS the double-run lane (completing the spec's [4,4]). EMPIRICAL FLAG (loud via the `lava_absent` payload): origin worlds generate ZERO lava tiles, so the LAVA four resolve `NO_ANCHOR` — contract-legal while the frozen "all seven from origin" wording holds for CANDIDATES only; a re-pin is a spec-level call. NO new baselines (31/32 are Build 1; 33 reserved for Build 3). **Build 3 (chaining + save v5) NOT STARTED.**
...
**Work**:
- **Landmarks (Build 1 — LANDED 2026-07-29)**: multi-tile structure generator (the current world places only single-tile props — `biome_defs.gd:64-94`). Pokémon Mansion (key item + statue switch puzzle), desert Ruins (glowing statues, high-level mons like Volcarona), Heart Tower. Map the dormant `PKMNMANSION`/`RUINS_*` encounter tokens (`biome_encounters.gd:14`).
- **Legendary placement (Build 2 — LANDED 2026-07-30)**: the frozen seven (MEWTWO, REGIROCK, REGICE, REGISTEEL, REGIELEKI, REGIDRAGO, REGIGIGAS) as STATIONARY encounters — NEVER random encounters — exactly one of each per world whose ring-≥60 band has the matching biome (flagged SNOW/LAVA affinity: REGICE/REGIELEKI/REGIGIGAS→SNOW, REGIROCK/REGISTEEL/REGIDRAGO/MEWTWO→LAVA); pure `_mix` anchors (`legendary_placement.gd`, NO rng). The legendary battle music (`music_router.gd:33`) finally gets its FIRST caller via the `battle_kind` seam (the forced-battle pending payload's `battle_kind`, default `"wild"`; a legendary static sets `"legendary"`; `main.gd:92` → `play_battle_track`). Gone-for-good per world on catch/KO (flagged PORT DECISION inverting the original's re-battleable stationary legendaries; removals persist via the v4-additive `legendary_removals` key, NO `SAVE_VERSION` bump); a white-out leaves the mon standing + re-battleable (HP/stat persistence rides the entity). Witnessed by `legendary_spawn` (ring-gated spawn proof, extends `biome_probe`). EMPIRICAL FLAG: generated origin worlds reach SNOW but NOT LAVA, so the LAVA four currently resolve `NO_ANCHOR` (the frozen "all seven from origin" holds for candidate tiles only; suite-witnessed via `lava_absent`; a re-pin is a spec-level call). HO-OH/LUGIA + ENTEI/RAIKOU/SUICUNE DEFERRED (no Phase-7 placement).
- **World chaining (Build 3 — pending)** (highest risk — spec separately within the phase): surf/fly off a map edge generates an adjacent world; Teleport Beacons (Phase 4 way-stone tech) at edges. **Save schema v5**: world identity (`world_id`) prefixes overrides and campsite anchors; migration from v4. Determinism: per-world seeds derived from a root seed so chains are reproducible.
