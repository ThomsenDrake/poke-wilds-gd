...
`render_introspection.gd` collects labels (`UiRenderModel.visible_labels` + `ink_rect` + `map_region`), draw order (composes `world_draw_order.gd`), palettes (manual distinct-color scan per region — `Image.get_used_colors()` does not exist on Godot 4.6.1), and expected regions (`UiRenderModel.expected`); `snapshot_capture.gd` writes the canonical sidecar; `visual_sweep_baselines.gd` commits baseline sidecars. `visual_region_diff.py`: region masks from committed sidecars — **canary rect at ~0 tolerance (first red-tier coded region)**, ink/string regions near-zero, known-dynamic zones (cursor model cell + live cursor; the pair's row rect is deliberately NOT masked — it would cover label ink) masked — global 0.5%/tolerance-8 kept as backstop; cluster bboxes + 32×18 ASCII grid + triptych via `png_canvas.py`; exit-code contract unchanged. `visual_explain.py`: sidecar↔cluster join → `label_moved / label_overlap_sprite / canary_absent / palette_dropped / region_ink_lost / unexplained`. `check_repo_contracts.py` gains the region-coverage + stdlib-only guards.
...
**Status (2026-07-21): machinery + baseline landed; all four battle states flipped on recorded evidence (Graduate phase complete).**
- `tools/graduation_ledger.py` (`record`/`status`/`calibration`, stdlib-only) landed: it banks each windowed `ui_render_audit` run — head_sha, boot-delimited session identity (`start_line`, `boot_ts_msec`), per-state finding counts with ≤8 sample payloads, the `text_oracle_passed` payload — into the **committed evidence binder** `docs/generated/graduation-ledger.json` (schema `graduation-ledger/1`, threshold 5). Streaks and flippability are COMPUTED from recorded entries, never narrated; `status` prints per-state streak N/5 with the exact blocker (missing predecessor flips, missing BOX judgment, or stale evidence — streak head_sha ≠ current HEAD — showing both shas) and an explicit next step. **No backfill** of the historical sessions: past reports are single-slot (overwritten) and `ts_msec` is a boot clock with no wall-clock join, so head_sha is unrecoverable — backfilling would assert, not record; history is citable in vision-fidelity.md prose only.
- Flip order (from `UiRenderModel.expected()`): battle_moves + battle_item FIRST (ANCHOR, exact integer pens, glyph-template-matched); battle_action NEXT (lint cleanliness on ACTION_ROWS — FIGHT/PKMN/ITEM/RUN are baked art, no glyph basis); battle_message LAST (BOX mode, engine-owned ~1px pen offset, glyph match excluded by design — flips only with a documented judgment recorded in the flip entry, else DEFERRED with streak progress banked). menu/party/bag NEVER flip (no model strings; a future author must not add menu keys). Each flip graduates the state's entire pixel lint (glyph + lint findings go red together) and edits ONLY `GRADUATED_STATES` at `ui_render_audit.gd:16` (in-place, 0 added lines) — the flip changes the harness's tiering, not the game.
- Calibration baseline landed with the machinery: precision/recall computed via Lane-4 `finding_id` and state+kind+region joins (`visual_explain.rects_overlap` is the single geometry home) over the quarantine→graduation pipeline — the free VLM calibration loop, no new engine events. Honest zero-denominators: cycle 1 renders "undefined (0 of 0 — no quarantine findings in the window)", never 0.0/1.0. The exit-criterion "trending up across two cadences" is met as **machinery + baseline + documented cadence** (weekly, aligned to the legibility-garden workflow, Mon 14:00 UTC): a two-cycle TREND requires the next cycle and is never claimed from one.
- The optional `vision` extra landed: `pyproject.toml` (uv, per user convention; `dependencies=[]` — core stays dependency-free) + `[project.optional-dependencies] vision = ["scikit-image>=0.22"]`, and `tools/vision_metrics.py` (SSIM-map corroboration) behind its own registry entry and the ONE documented stdlib-guard exemption (file-scoped, with a hardening clause so no core tool can STATICALLY import it — an AST-level source check; a runtime `importlib` load is out of scope for the static guard). Importable-graceful (absent → skipped section, never an error), quarantine-forever, structurally never a gate (no runner wiring; windowing dilutes exactly the sprite/glyph-scale defects the coded oracles catch at exact pixels).
- **The four optional spikes stay DEFERRED — explicitly off the critical path, not built**: Movie Writer PNG-burst motion lane; DAP `godot/put_msg` bridge; ScriptBacktrace (#91006) structured error frames; and the live NDJSON-over-TCP introspection endpoint. The endpoint is specced as "built ONLY IF in-process sidecars prove insufficient", and they have not — in-process sidecars + explainable per-region diff + trace correlation deliver the structured-observation value with zero new runtime surface. Each would ship behind its own registry entry if ever adopted; none gates CI.
- Per-state flip outcomes are recorded in the ledger's `flips[]`/`runs[]` (Graduate phase 2026-07-21, HEAD 7b733946: all four battle states flipped on ledger runs 1-5 — moves + item anchor, action lint, message box with the documented judgment; no deferrals) and rendered in the legibility report's "Graduation & calibration" section; exit criterion 4 (seeded-defect proof per flipped state, same defect class stays quarantine in non-graduated states, clean runs stay green, perturbations reverted) is executed with temporary perturbations and git-diff revert proof.
