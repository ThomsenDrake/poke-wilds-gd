...
| `quarantine_finding` | `SmokeScenarios` | A quarantined heuristic pixel check reported a possible visual defect; payload carries state, kind (`low_ink`/`forbidden_ink`/`garble`/`lint_unavailable`), and region. Never fails a scenario until graduated. The playtest runner additionally records kind `region_drift` (non-red per-region drift and unexplained change clusters from the region diff) in the playtest-report quarantine section — report-tier, not emitted in the JSONL trace. |
