...
4. **Freshness / provenance** — the report carries `head_sha`/`godot_version`/`window`/`renderer` stamps; each sidecar's `capture_env` duplicates renderer/adapter/godot_version per shot for baseline provenance. A baseline captured under a different binary or driver must not be diffed — IMPLEMENTED (Workstream L.1): `tools/verify_all.py` mechanizes the refuse-on-mismatch (R1 `head_sha`==git HEAD; R3 `godot_version`/`renderer`/`window` vs the baseline `capture_env` + the canonical 1152×648 window), REFUSING (exit 2) rather than diffing across a provenance mismatch.
