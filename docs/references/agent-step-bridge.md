...
- `PLAY_AGENT_PLANNER_CMD` — optional CLI; stdin is one observation JSON, stdout is one command JSON. Default explorer is observation-driven: `boot_new_game`, harvest (cut/smash/dig) with `action_a`, toggle `build_toggle` and try to place, then walk onto a nearby overworld mon, fight, and try a capture. After that minimum it opens menus/camp overlays, lingers, and keeps playing until the 96-turn / 180s cap.
