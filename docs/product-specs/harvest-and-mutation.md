...
- Harvested tiles change permanently: cut and smashed tiles are cleared (prop removed, walkable), dug tiles lose tall grass and their encounter flag. Re-harvesting a harvested tile is refused. Landmark tiles are NEVER harvestable: `action_for_tile` returns no action for any tile carrying a `landmark_id` — footprint tiles are immutable world features (world-depth.md § Landmarks), so no cut/dig/smash mutation ever lands on a landmark floor or prop (the build-protection guard's harvest-side sibling).
