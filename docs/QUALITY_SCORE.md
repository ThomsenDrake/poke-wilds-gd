...
| `vision_fidelity` | `app` | `vision` | 3 | 2 | 3 | 2 | Capture honesty for windowed verification: a `frame_post_draw` readback guard added after (never replacing) scenario settle waits, a capture validity oracle (undersize/blank/uniform/magenta) with transport-vs-regression classification, a quarantine-tier duplicate-capture hook, a root-viewport-crop fallback for the Godot 4.6 magenta SubViewport regression (#115402), and head_sha/godot_version/window/renderer stamps on the playtest report. Region diffing, sidecar metadata, graduated pixel gates, and the text oracle land in later slices, which is why validation and product completeness start at 2. |
