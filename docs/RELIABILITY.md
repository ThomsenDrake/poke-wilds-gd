...
Source paths: tools/check_repo_contracts.py, tools/check_architecture.py, tools/check_quality_docs.py, tools/godot_dap_smoketest.py, tools/determinism_verify.py
...
Baseline regeneration (windowed; run from the repo root, bound each windowed run with a 600000 ms timeout and retry once if a fully-occluded macOS window stalls it): when a sweep's shots legitimately change — intentional art or script changes, or a determinism pin that measurably changes pixels — regenerate all 16 baselines with `python3 tools/run_playtests.py --scenario visual_sweep_update` (windowed-subprocess transport; do NOT set `PLAYTEST_FORCE_HEADLESS` — that turns the scenario into an honest skip, not a regeneration). Stale PNGs are pruned automatically. Then refresh the tracked import metadata with a headless import pass, `/Applications/Godot.app/Contents/MacOS/Godot --headless --path . --import`: the 16 `docs/generated/visual-baselines/*.png.import` files ARE git-tracked, so review their diff and keep it in the same change (uids persist since source paths are unchanged). Verify the regeneration with TWO consecutive windowed `python3 tools/run_playtests.py --scenario visual_sweep` runs — both must emit `visual_sweep_passed` with max drift 0.0 (bit-identical vs the freshly committed baselines); optionally `PLAYTEST_CAPTURE_DUPCHECK=1` proves the intra-run noise floor is zero. `tools/determinism_verify.py cmp --a .godot-smoke/shots --b docs/generated/visual-baselines` byte-compares two shot directories without decoding.
