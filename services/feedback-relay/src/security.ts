...
import type { BuildMetadata, CaptureMetadata, GameMetadata, ReportMetadata, RuntimeMetadata } from "./types";
...
export function validateMetadata(value: unknown): ReportMetadata {
  if (!isObject(value)) throw badRequest("metadata_not_object");
  const fields = validateReportFields(value);
  const bundleSha256 = String(value.bundle_sha256 ?? "");
  const bundleBytes = Number(value.bundle_bytes);
  if (!SHA256.test(bundleSha256)) throw badRequest("invalid_bundle_hash");
  if (!Number.isInteger(bundleBytes) || bundleBytes < 1 || bundleBytes > MAX_COMPRESSED_BYTES) {
    throw badRequest("invalid_bundle_size");
  }
  return { ...fields, bundle_sha256: bundleSha256, bundle_bytes: bundleBytes };
}
...
export async function inspectBundle(bytes: Uint8Array, metadata: ReportMetadata): Promise<Record<string, unknown>> {
  ...
  if (entries.length < REQUIRED.size || entries.length > REQUIRED.size + OPTIONAL.size) throw badRequest("invalid_entry_count");
  for (const name of REQUIRED) if (!names.has(name)) throw badRequest(`missing_${name}`);
  for (const name of names) if (!REQUIRED.has(name) && !OPTIONAL.has(name)) throw badRequest("unexpected_entry");
  if (metadata.capture.screenshot_available !== names.has("screenshot.png")) throw badRequest("capture_screenshot_mismatch");
  if (entries.reduce((sum, entry) => sum + entry.uncompressed, 0) > MAX_UNCOMPRESSED_BYTES) throw payloadTooLarge("bundle_uncompressed_too_large");
  let unpacked: Record<string, Uint8Array>;
  try {
    unpacked = unzipSync(bytes);
  } catch (_error) {
    throw badRequest("invalid_zip");
  }
  if (Object.keys(unpacked).length !== entries.length) throw badRequest("invalid_unpacked_entries");
  for (const entry of entries) {
    const payload = unpacked[entry.name];
    if (!payload || payload.byteLength !== entry.uncompressed) throw badRequest(`zip_size_mismatch:${entry.name}`);
    if (crc32(payload) !== entry.crc32) throw badRequest(`zip_crc_mismatch:${entry.name}`);
  }
  const raw = unpacked["report.json"];
  if (!raw || raw.byteLength > MAX_METADATA_BYTES) throw badRequest("invalid_report_json");
  let report: Record<string, unknown>;
  try {
    report = JSON.parse(strFromU8(raw)) as Record<string, unknown>;
  } catch (_error) {
    throw badRequest("invalid_report_json");
  }
  validateManifest(report, names);
  if (!manifestAgrees(report, metadata)) throw badRequest("manifest_mismatch");
  ...
  if (artifacts.length !== expectedArtifacts.size) throw badRequest("artifact_manifest_mismatch");
  for (const artifact of artifacts) {
    const name = String(artifact.path ?? "");
    const payload = unpacked[name];
    if (!expectedArtifacts.delete(name) || !payload) throw badRequest("artifact_manifest_mismatch");
    if (artifact.bytes !== payload.byteLength ||
        await sha256Hex(payload) !== artifact.sha256) throw badRequest(`artifact_hash_mismatch:${name}`);
  }
  if (expectedArtifacts.size) throw badRequest("artifact_manifest_mismatch");
  return report;
}
...
function validateManifest(report: Record<string, unknown>, names: Set<string>): void {
  ...
  if (typeof report.created_at_utc !== "string" || !report.created_at_utc.trim() ||
    ...
    throw badRequest("invalid_created_at_utc");
  }
  if (!Array.isArray(report.artifacts)) throw badRequest("invalid_artifacts");
  for (const artifact of report.artifacts) {
    if (!isObject(artifact) || typeof artifact.path !== "string" || !isSafeName(artifact.path) ||
        ...
        typeof artifact.truncated !== "boolean") throw badRequest("invalid_artifact");
  }
  const capture = report.capture as Record<string, unknown>;
  if (capture.screenshot_available !== names.has("screenshot.png")) throw badRequest("capture_screenshot_mismatch");
}

type ReportFields = Omit<ReportMetadata, "bundle_sha256" | "bundle_bytes">;

function validateReportFields(data: Record<string, unknown>): ReportFields {
  if (data.schema_version !== 1) throw badRequest("unsupported_schema");
  ...
  if (!/^[0-9a-f]{32}$/.test(installId)) throw badRequest("invalid_install_id");
  if (!isBuild(data.build)) throw badRequest("invalid_build");
  if (!isRuntime(data.runtime) || !isGame(data.game)) throw badRequest("invalid_runtime_or_game");
  if (!isCapture(data.capture, data.game)) throw badRequest("invalid_capture");
  return {
    schema_version: 1, report_id: reportId, message: data.message,
    tester_id: testerId, install_id: installId,
    build: data.build, runtime: data.runtime, game: data.game, capture: data.capture,
  };
}

function isBuild(value: unknown): value is BuildMetadata {
...
function isRuntime(value: unknown): value is RuntimeMetadata {
...
function isGame(value: unknown): value is GameMetadata {
  return isObject(value) && typeof value.current_screen === "string" && value.current_screen.trim().length > 0 &&
    ...
    isObject(value.bag) && isJsonValue(value.party) && isJsonValue(value.bag) && typeof value.battle_active === "boolean";
}

function isCapture(value: unknown, game: unknown): value is CaptureMetadata {
...
function centralDirectoryEntries(bytes: Uint8Array): ZipEntry[] {
  if (bytes.byteLength < 22) throw badRequest("invalid_zip");
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const eocd = findEocd(view, bytes.byteLength);
  if (eocd >= 20 && view.getUint32(eocd - 20, true) === 0x07064b50) throw badRequest("zip64_unsupported");
  if (view.getUint16(eocd + 4, true) !== 0 || view.getUint16(eocd + 6, true) !== 0) throw badRequest("zip_multidisk");
  const entriesOnDisk = view.getUint16(eocd + 8, true);
  const entriesTotal = view.getUint16(eocd + 10, true);
  const directorySize = view.getUint32(eocd + 12, true);
  const directoryOffset = view.getUint32(eocd + 16, true);
  if (entriesOnDisk !== entriesTotal) throw badRequest("zip_multidisk");
  if (entriesTotal === 0xffff || directorySize === 0xffffffff || directoryOffset === 0xffffffff) throw badRequest("zip64_unsupported");
  const directoryEnd = directoryOffset + directorySize;
  if (directoryEnd !== eocd || directoryEnd < directoryOffset) throw badRequest("invalid_zip_directory");
  const entries: ZipEntry[] = [];
  let offset = directoryOffset;
  for (let index = 0; index < entriesTotal; index++) {
    if (offset + 46 > directoryEnd || view.getUint32(offset, true) !== CENTRAL_SIGNATURE) throw badRequest("invalid_zip_directory");
    ...
    if (entryEnd > directoryEnd || diskStart !== 0) throw badRequest("invalid_zip_directory");
    if (compressed === 0xffffffff || uncompressed === 0xffffffff || localOffset === 0xffffffff || hasZip64Extra(bytes, offset + 46 + nameLength, extraLength)) {
      throw badRequest("zip64_unsupported");
    }
    if (flags & ~0x0800) throw badRequest("unsupported_zip_flags");
    if (method !== 0 && method !== 8) throw badRequest("unsupported_zip_method");
    const unixFileType = (externalAttributes >>> 16) & 0xf000;
    if ((externalAttributes & 0x10) !== 0 || (unixFileType !== 0 && unixFileType !== 0x8000)) {
      throw badRequest("unsafe_entry_type");
    }
    const name = new TextDecoder().decode(bytes.subarray(offset + 46, offset + 46 + nameLength));
    if (!isSafeName(name)) throw badRequest("unsafe_entry_name");
    entries.push({ name, crc32, compressed, uncompressed, localOffset, flags, method });
    offset = entryEnd;
  }
  if (offset !== directoryEnd || entries.length !== entriesTotal) throw badRequest("invalid_zip_directory");
  if (new Set(entries.map((entry) => entry.name)).size !== entries.length) throw badRequest("duplicate_zip_entry");
...
function findEocd(view: DataView, length: number): number {
  ...
  throw badRequest("missing_zip_eocd");
}
...
function hasZip64Extra(bytes: Uint8Array, offset: number, length: number): boolean {
  ...
  while (cursor + 4 <= end) {
    ...
    cursor += 4;
    if (cursor + size > end) throw badRequest("invalid_zip_extra");
    if (id === 0x0001) return true;
    cursor += size;
  }
  if (cursor !== end) throw badRequest("invalid_zip_extra");
  return false;
}
...
function validateLocalHeaders(bytes: Uint8Array, view: DataView, entries: ZipEntry[], directoryOffset: number): void {
  ...
  for (const entry of entries) {
    ...
    if (offset + 30 > directoryOffset || view.getUint32(offset, true) !== LOCAL_SIGNATURE) throw badRequest("invalid_local_header");
    ...
    if (dataEnd > directoryOffset || flags !== entry.flags || method !== entry.method || crc32 !== entry.crc32 ||
        compressed !== entry.compressed || uncompressed !== entry.uncompressed) throw badRequest("local_header_mismatch");
    const name = new TextDecoder().decode(bytes.subarray(offset + 30, offset + 30 + nameLength));
    if (name !== entry.name || hasZip64Extra(bytes, offset + 30 + nameLength, extraLength)) throw badRequest("local_header_mismatch");
    ranges.push([offset, dataEnd]);
  }
  ranges.sort((left, right) => left[0] - right[0]);
  for (let index = 1; index < ranges.length; index++) if (ranges[index][0] < ranges[index - 1][1]) throw badRequest("overlapping_zip_entries");
}
