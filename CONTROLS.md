...
- **This slice**: the battle menu offers Fight, Item, and Run only (no party switching); breeding, shinies, habitat drops, fishing, evolution-stone bag use, AND stone ACQUISITION are in (Phase 5; the faithful Beach-Dig Water Stone plus the flagged port-addition sources above — Ice/Dawn Stones stay grant-only); AND visible overworld Pokémon are in (Phase 6) — roaming mons with four dispositions, Charm-recruit and Attack-engage now have real targets, strewn wild eggs with the faithful TAKE-vs-Attack binary, and the flagged port-invention nest + Alpha guardian model. World chaining (multiple worlds, edge beacons, legendaries, landmarks) is next.
